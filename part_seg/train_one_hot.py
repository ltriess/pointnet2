...
def train():
    with tf.Graph().as_default():
        with tf.device("/gpu:" + str(GPU_INDEX)):
            pointclouds_pl, labels_pl, cls_labels_pl = MODEL.placeholder_inputs(
            ...
            pred, end_points = MODEL.get_model(
                pointclouds_pl, cls_labels_pl, is_training_pl, bn_decay=bn_decay
            )
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_cls_label = get_batch(
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        cur_batch_data, cur_batch_label, cur_batch_cls_label = get_batch(
        ...
        for l in range(NUM_CLASSES):
            ...
            total_correct_class[l] += np.sum(
                (cur_pred_val == l) & (cur_batch_label == l)
            )
    ...
    log_string(
        "eval mean loss: %f" % (loss_sum / float(len(TEST_DATASET) / BATCH_SIZE))
    )
