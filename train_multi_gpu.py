...
parser.add_argument(
    "--num_gpus", type=int, default=1, help="How many gpus to use [default: 1]"
)
parser.add_argument(
    "--model",
                ...
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device("/gpu:%d" % (i)), tf.name_scope(
                        ...
                        label_batch = tf.slice(
                            labels_pl, [i * DEVICE_BATCH_SIZE], [DEVICE_BATCH_SIZE]
                        )

                        pred, end_points = MODEL.get_model(
                            pc_batch, is_training=is_training_pl, bn_decay=bn_decay
                        )
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    while TEST_DATASET.has_next_batch():
        ...
        for i in range(0, bsize):
            bl = batch_label[i]
            total_seen_class[bl] += 1
            total_correct_class[bl] += pred_val[i] == bl
