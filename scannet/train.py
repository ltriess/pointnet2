...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        correct = np.sum(
            (pred_val == batch_label) & (batch_label > 0) & (batch_smpw > 0)
        )  # evaluate only on 20 categories but not unknown
        total_correct += correct
        ...
        for c in range(NUM_CLASSES):
            total_seen_class[c] += np.sum((batch_label == c) & (batch_smpw > 0))
            total_correct_class[c] += np.sum(
                (pred_val == c) & (batch_label == c) & (batch_smpw > 0)
            )
        ...
        for b in range(batch_label.shape[0]):
            ...
            total_correct_vox += np.sum(
                (uvlabel[:, 0] == uvlabel[:, 1]) & (uvlabel[:, 0] > 0)
            )
            total_seen_vox += np.sum(uvlabel[:, 0] > 0)
            tmp, _ = np.histogram(uvlabel[:, 0], range(22))
            labelweights_vox += tmp
            for c in range(NUM_CLASSES):
                total_seen_class_vox[c] += np.sum(uvlabel[:, 0] == c)
                total_correct_class_vox[c] += np.sum(
                    (uvlabel[:, 0] == c) & (uvlabel[:, 1] == c)
                )

    log_string("eval mean loss: %f" % (loss_sum / float(num_batches)))
    log_string(
        "eval point accuracy vox: %f" % (total_correct_vox / float(total_seen_vox))
    )
    log_string(
        "eval point avg class acc vox: %f"
        % (
            np.mean(
    ...
    for c in range(1, NUM_CLASSES):
        per_class_str += "class %d weight: %f, acc: %f; " % (
            c,
            labelweights_vox[c - 1],
            total_correct_class[c] / float(total_seen_class[c]),
        )
...
def eval_whole_scene_one_epoch(sess, ops, test_writer):
    ...
    log_string(
        "eval whole scene point accuracy vox: %f"
        % (total_correct_vox / float(total_seen_vox))
    )
    log_string(
        "eval whole scene point avg class acc vox: %f"
        % (
            np.mean(
                np.array(total_correct_class_vox[1:])
                / (np.array(total_seen_class_vox[1:], dtype=np.float) + 1e-6)
    ...
    log_string(
        "eval whole scene point accuracy: %f" % (total_correct / float(total_seen))
    )
    log_string(
    ...
    for l in range(1, NUM_CLASSES):
        per_class_str += "class %d weight: %f, acc: %f; " % (
            l,
            labelweights_vox[l - 1],
            total_correct_class_vox[l] / float(total_seen_class_vox[l]),
        )
    log_string(per_class_str)
    EPOCH_CNT += 1
    return caliacc
